...
def index(request):
    ...
    user_following_list = [followed_user.user for followed_user in user_following]
    feed = [Post.objects.filter(user=user) for user in user_following_list]
    feed_list = [post for queryset in feed for post in queryset]
    ...
    for ids in username_profile:
        users_profile = Profile.objects.filter(id_user=ids)
        username_profile_list.append(users_profile)
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                #Log user in a redirect to settings page
